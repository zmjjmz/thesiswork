...
def build_te_embedder_c1d(n_scales=4):
    # assume input is a 1000-d by n_scales trailing edge of (normalized) curvatures
    inp = ll.InputLayer(shape=(None, n_scales, 1000), name='input')
    # alternate pooling and conv layers to minimize parameters

    conv1 = ll.Conv1DLayer(inp, num_filters=16, filter_size=3, pad='same', W=Orthogonal(), nonlinearity=rectify, name='conv1')
    mp1 = ll.MaxPool1DLayer(conv1, 2, stride=2, name='mp1') # now down to 500
    bn1 = ll.BatchNormLayer(mp1, name='bn1')
    conv2 = ll.Conv1DLayer(bn1, num_filters=16, filter_size=3, pad='same', W=Orthogonal(), nonlinearity=rectify, name='conv2')
    mp2 = ll.MaxPool1DLayer(conv2, 2, stride=2, name='mp2') # now down to 250
    bn2 = ll.BatchNormLayer(mp2, name='bn2')
    conv3 = ll.Conv1DLayer(bn2, num_filters=16, filter_size=3, pad='same', W=Orthogonal(), nonlinearity=rectify, name='conv3')
    mp3 = ll.MaxPool1DLayer(conv3, 2, stride=2, name='mp3') # now down to 125
    bn3 = ll.BatchNormLayer(mp3, name='bn3')
    dp0 = ll.DropoutLayer(bn3, p=0.5, name='dp0')
    # now let's do a few FC layers
    fc1 = ll.DenseLayer(dp0, num_units=2048, nonlinearity=rectify, name='fc1')
...
def create_network_fn(embed_layer):
    # just take the embedder output, and make a tfn giving it some tensor3
    X = T.tensor3()
...
def make_batch_pairs(train_pairs, bslice, dataset):
    ...
    batches = {'te1':dataset['Xall'][pairs1],
               'te2':dataset['Xall'][pairs2],
...
augmenting_batch_loader_pair = partial(augmenting_batch_loader, keys=['te1', 'te2', 'y'])
nonaugmenting_batch_loader_pair = partial(nonaugmenting_batch_loader, keys=['te1', 'te2', 'y'])
...
if __name__ == "__main__":
    ...
    dset = load_dataset(join(dataset_loc, "Flukes/patches/%s" % dset_name), normalize_method=None)
    ...
    epoch_losses = []
    batch_losses = []
    embedder = build_te_embedder_c1d()
